...
def get_openai_api_key():
    ...
    if not OPENAI_API_KEY_SECRET_ARN:
        raise ValueError("OPENAI_API_KEY_SECRET_ARN or OPENAI_API_KEY must be configured.")

    import boto3

    secrets_client = boto3.client("secretsmanager")
    secret_value = secrets_client.get_secret_value(SecretId=OPENAI_API_KEY_SECRET_ARN)
    _OPENAI_API_KEY = secret_value["SecretString"]
    return _OPENAI_API_KEY
...
def handler(event, context):
    try:
        ...
        retrieved_context = retrieve(question, documents, top_k=top_k)
